  ...
  function render(containerElement) {
    ...
    var html = `
      <div class="page-title-bar">
        <span>Danh Mục Nghiệp Vụ - Cơ Sở Dữ Liệu</span>
      </div>

      <div class="card" style="animation: slideUp 0.3s ease forwards; display: flex; flex-direction: column; height: calc(100vh - var(--header-height) - 100px);">
        
        <!-- THANH CÔNG CỤ -->
        <div class="card-header" style="background:#f8fafc; border-bottom: 1px solid var(--color-border); padding: 12px 24px;">
          <div class="button-bar" style="border: none; padding: 0; box-shadow: none; background: transparent;">
            <button class="btn btn-tool" onclick="CategoriesPage.add()"><span class="material-symbols-outlined">add</span> Thêm mới</button>
            <button class="btn btn-tool" onclick="CategoriesPage.edit()"><span class="material-symbols-outlined">edit</span> Sửa</button>
            <div class="divider"></div>
            <button class="btn btn-tool text-danger" onclick="CategoriesPage.remove()"><span class="material-symbols-outlined">delete</span> Xóa</button>
            <div class="divider"></div>
            <button class="btn btn-tool" onclick="UIToast.show('Mở form Lọc chi tiết')"><span class="material-symbols-outlined">filter_list</span> Lọc</button>
            <button class="btn btn-tool" onclick="UIToast.show('In dữ liệu danh mục hiện hành')"><span class="material-symbols-outlined">print</span> In</button>
            <div style="flex:1"></div>
            <button class="btn btn-tool" onclick="window.location.hash='#/dashboard'"><span class="material-symbols-outlined">close</span> Đóng</button>
          </div>
        </div>

        <div style="display: flex; flex: 1; flex-wrap: wrap; overflow: hidden; position: relative;" class="categories-layout">
          <style>
            @media(min-width: 769px) {
              #btn-mobile-open-tree, #btn-mobile-close-tree { display: none !important; }
            }
            @media(max-width: 768px) {
              #btn-mobile-open-tree, #btn-mobile-close-tree { display: block !important; }
              .categories-layout > .tree-column { display: none; }
              .categories-layout.show-tree > .tree-column { display: flex !important; position: absolute; z-index: 10; background: white; width: 100%; height: 100%; border-right: none; }
            }
          ...
          <!-- CỘT TRÁI: CÂY -->
          <div class="tree-column" style="flex: 0 0 300px; border-right: 1px solid var(--color-border); flex-direction: column; overflow-y: auto; background: #fff;">
            <div style="padding: 16px; border-bottom: 1px solid var(--color-border); display: flex; justify-content: space-between; align-items: center; position: sticky; top:0; background: #fdfdfd; z-index: 2;">
              <span style="font-weight: 700; font-size: 15px; color: var(--color-text-dark);">Phân nhóm Danh mục</span>
              <button class="btn-tool" style="padding:4px;" id="btn-mobile-close-tree" onclick="document.querySelector('.categories-layout').classList.remove('show-tree')">
                <span class="material-symbols-outlined">close</span>
              </button>
            </div>
            <div style="padding: 12px;" id="categories-tree-container"></div>
          </div>

          <!-- CỘT PHẢI: GIAO DIỆN ĐỘNG THEO MODULE -->
          <div style="flex: 1; display: flex; flex-direction: column; overflow: hidden; background: #fff; min-width: 300px;">
            <div style="padding: 16px; border-bottom: 1px solid var(--color-border); display: flex; align-items: center; gap: 12px; background: #fff; z-index: 2; box-shadow: 0 2px 4px -2px rgba(0,0,0,0.02)">
              <button class="btn-tool" style="padding:4px;" id="btn-mobile-open-tree" onclick="document.querySelector('.categories-layout').classList.add('show-tree')">
                <span class="material-symbols-outlined">menu_open</span>
              </button>
              <div style="display: flex; flex-direction: column;">
                <span style="font-weight: 700; font-size: 18px; color: var(--color-primary);" id="current-category-title">Vui lòng chọn danh mục...</span>
              </div>
            </div>
            
            <!-- VÙNG RENDER CONTENT -->
            <div id="category-content-container" style="flex: 1; display: flex; flex-direction: column; overflow: hidden; min-height: 0;">
              <div style="text-align:center; padding: 60px; color: var(--color-text-secondary); display:flex; flex-direction: column; align-items: center; justify-content: center; height:100%;">
                <span class="material-symbols-outlined" style="font-size: 64px; color: #cbd5e1; margin-bottom: 16px;">account_tree</span>
                <span>Click vào một loại danh mục bên gốc trái để bắt đầu thiết lập.</span>
              </div>
            </div>
          </div>
    ...
    // Nút toggle menu tree được xử lý bằng CSS hoàn toàn
  }
